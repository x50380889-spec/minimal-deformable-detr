...
| --- | --- | --- | --- |
| 教师（Deformable DETR，3.85M） | 0.192 | 5.2 | 3.85M |
| 学生·裸训练（MobileNetV3-Small） | 0.072 | 12.9 | 1.37M |
| 学生·蒸馏后 | 0.067 | 12.7 | 1.37M |

- **速度**：蒸馏学生比教师快约 **2.4 倍**（CPU 单帧，FPS 受机器负载影响）；
- **体积**：参数量缩小 **2.8 倍**；
- **精度**：蒸馏学生相对教师下降约 **12.6 个百分点**——这是小容量学生在
  合成小目标数据上的真实代价，也是精度-速度权衡曲线的量化证据；
- **蒸馏价值**：与裸训练学生精度相当（±1pt，噪声范围内），但**前期收敛显著更快**
  （见 `training_logs/loss_curves.png`：epoch 7 蒸馏学生 mAP≈0.065，裸学生≈0.026）
  ——KD 用教师的先验加速了小模型的学习。

> 坦白说明：合成数据噪声大、教师本身只有约 19% 的
...
1. **为什么选 Deformable DETR 而非 YOLO**：企业考察底层功底；可变形注意力是
   DETR 家族最有代表性的算子，能讲清「稀疏采样如何省算力」；
2. **从零实现的边界**：不用 mmcv，投影/采样/加权全部手写，并有与官方 API
   的对拍单测——证明的不是「会调包」而是「懂数学实现」；
3. **蒸馏为什么有效**：教师在小目标上的「困难样本区分度」通过软标签与
   特征对齐传入学生，等价于把教师的多尺度空间推理压缩进 MobileNet；
4. **落地视角**：FPS 基准用 CPU 单帧（贴近端侧）、报告参数量与速度/精度权衡，
   并给出 `make train` 一键复现——工程可复现性本身就是加分项。

---

## 7. 局限与后续方向

- 当前数据为程序化合成缺陷（保证可复现），真实产线数据上需要标注与域适应；
- 可进一步做：量化感知训练（INT8）、TensorRT/NCNN 部署、教师集成蒸馏
  （多个教师）、以及针对小目标的尺度感知损失加权。
